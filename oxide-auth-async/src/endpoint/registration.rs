...
use oxide_auth::code_grant::registration::{Error as RegistarError, Endpoint as RegistarEndpoint, Request as RegistarRequest, RegistarClient, Error};
use super::*;
use oxide_auth::endpoint::{Endpoint, WebRequest, QueryParameter, WebResponse, OAuthError, Scopes, OwnerSolicitor, Template, InnerTemplate};
use std::borrow::Cow;
use oxide_auth::endpoint::Extension as SuperExtension;
use oxide_auth::primitives::registrar::{Client, ClientType, ClientMap, Registrar};
use oxide_auth::primitives::issuer::Issuer;
use oxide_auth::code_grant::error::{AccessTokenError, AuthorizationError};
use oxide_auth::code_grant::resource::Error as ResourceError;
...
/*
        ...
        let r = match r.regist(private_client){
            Ok(r) => {println!("registered."); r},
            Err(e) => {println!("not registered."); "".to_string()}
        };
